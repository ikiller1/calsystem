...
if(true)
{
	...
	else
	{
		...
		else
		{
			...
			if (file_exists($tag. $_FILES["file"]["name"]))
			{
				$code=1002;
			...
			else
			{
				...
				move_uploaded_file($_FILES["file"]["tmp_name"], $tag ."/". $_FILES["file"]["name"]);
				//echo "文件存储在: " . "filestorage/" .$tag."/". $_FILES["file"]["name"];
				$code=0;
				$msg="上传成功";
				$path="filestorage/" .$tag;
...
$result = array("code"=>$code,"msg"=>$msg,"path"=>$path,"filename"=>$filename);
